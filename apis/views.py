...
def render_basic_app_deploy_result_to_msg(deploy_result):
    ...
    for pgname, pg_result in proc_results['proc_deploy_success'].iteritems():
        msg += '    %s\n%s\n' % (pgname, render_podgroup_deploy_result_to_msg(pg_result))
...
def render_resource_deploy_result_to_msg(deploy_result):
    ...
    for riname, ri_result in instance_results.iteritems():
        msg += '%s\n%s\n' % (riname, render_basic_app_deploy_result_to_msg(ri_result))
    return msg
...
class AppApi:
    ...
    def list_repos(cls, open_auth, groups, options=None):
        try:
            apps = App.all()
            if open_auth:
                app_datas = [AppApi.render_repo_data(a.appname) for a in apps if AuthApi.verify_app_access(groups, a.appname)]
            else:
    ...
    def _app_deploy_thread(cls, token, app, meta_version):
        try:
            ...
            if app.get_app_type() != AppType.Resource:
                ...
                success, msg = AuthApi.create_resource_instance_group(token, app.appname)
                if not success:
                ...
                if not deploy_result.get("OK", False):
                    raise Exception("error deploying : %s" % render_app_deploy_result_to_msg(deploy_result))
        except Exception, e:
            client.captureException()
    ...
    def _get_configed_instances(cls, token, app, resources, update=False):
        ...
        for resourcename, resource_props in resources.iteritems():
            ...
            if resource is None or not resource.is_reachable():
                ...
                raise Exception("Error: Resource %s DoesNotExist" % resourcename)
            ...
            ConfigApi.construct_config_for_instance(token, resource, resource_instance)
    ...
    def create_repo(cls, access_token, appname, options=None):
        try:
            ...
            op_logger.info("REPOSIT: app %s reposited by %s" % (appname, AuthApi.operater))
    ...
    def delete_app(cls, appname, options=None):
        try:
            ...
            op_logger.info("DELETE: app %s deleted by %s" % (appname, AuthApi.operater))
            ...
            return (202, AppApi.render_repo_data(appname),
                    'delete app successfully.' if is_meta_empty else render_app_remove_result_to_msg(remove_results),
                    reverse('api_apps'))
        except Exception, e:
            client.captureException()
            return (500, None,
    ...
    def update_app(cls, access_token, appname, options=None):
        try:
            ...
            target_meta_version = options.get('meta_version', None) if isinstance(options, dict) else None
    ...
    def _update_resource_instance(cls, token, instance, target_meta_version):
        ...
        instance.default_image = Resource.get_instance_image(resourcename, instance.meta_version)

        clientname = Resource.get_clientappname_from_instancename(instance.appname)
    ...
    def render_proc_data(cls, appname, proc_lain_conf, proc_status=None, is_portal=False, client=None):
        ...
        if proc_status:
            ...
            try:
                ...
                data['cpu'] = int(proc_status['Status']['Spec']['Pod']['Containers'][0]['CpuLimit'])
                data['memory'] = humanfriendly.format_size(int(proc_status['Status']['Spec']['Pod']['Containers'][0]['MemoryLimit']))
                data['image'] = proc_status['Status']['Spec']['Pod']['Containers'][0]['Image']
            except:
                pass
        return data
        ...
        try:
            ...
            for proc in app.lain_config.procs.values():
                if proc.type != ProcType.portal:
                    ...
                    proc_datas.append(ProcApi.render_proc_data(appname, proc, pg_status))
        ...
        try:
            ...
            if pg_status:
                ...
                if verified_options.has_key('num_instances'):       
                    ...
                    deploy_result = app.podgroup_deploy(new_podgroup_spec, autopatch=False)
    ...
    def delete_app_proc(cls, appname, procname, options=None):
        try:
            ...
            if pg_status:
                ...
                podgroup_name = "%s.%s.%s" % (appname, proc.type.name, proc.name)
                ...
                if remove_result.status_code < 400:
                    return (202, ProcApi.render_proc_data(appname, proc),
                            render_podgroup_remove_result_to_msg(remove_result),
                            reverse('api_procs', kwargs={'appname': appname}))
                else:
...
class MaintainApi:
    ...
    def add_maintainer(cls, access_token, appname, username, role):
        try:
            success, msg = Group.add_group_member(access_token, appname, username, role)
            if success:
    ...
    def delete_maintainer(cls, access_token, appname, username):
        try:
            success, msg = Group.delete_group_member(access_token, appname, username)
            if success:
                return (204, None, msg,
                        reverse('api_maintainers', kwargs={'appname': appname}))
            else:
    ...
    def get_maintainers(cls, access_token, appname):
        try:
            ...
            if success:
                maintainer_datas = [MaintainApi.render_maintainer_data(appname, maintainer['name'], maintainer['role']) for maintainer in msg]
    ...
    def create_resource_instance_group(cls, access_token, appname):
        ...
        for resource_appname, resource_props in resources.iteritems():
            ...
            if not instance:
                ...
                success, msg = Group.create_group_for_resource_instance(access_token, resource_appname, instance_name)
                if not success:
                    instance.delete()
                    return False, msg
...
class ConfigApi:
    ...
    def _construct_config(cls, token, app, pg_name, base_image):
        ...
        new_release_image = cls.gen_release_image(app, base_image, config_tag, len(config_list))
    ...
    def get_config_image(cls, app, config_list, defined_secret_files, pg_name, timestamp):
        latest_config_tag = "%s-config-%s" % (pg_name.split(".")[-1], timestamp)
        if latest_config_tag not in app.registry_tags:
            Config.generate_config_image(config_list, defined_secret_files, app.appname, latest_config_tag)
    ...
    def gen_release_image(cls, app, base_image, config_tag, config_layer_count):
        if not base_image.startswith(PRIVATE_REGISTRY):
            logger.error("base_image format wrong, should start with %s, in fact %s" % (PRIVATE_REGISTRY, base_image))
            raise Exception("base_image format wrong, should start with %s, in fact %s" % (PRIVATE_REGISTRY, base_image))
