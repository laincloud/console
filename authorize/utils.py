...
def send_request(method, path, headers, json, params):
    response = requests.request(method, path, headers=headers, json=json, params=params)
    return response
...
def redirect_to_ui(token_json):
    ...
    response.set_cookie('ACCESS_TOKEN', value=token_json['access_token'], max_age=token_json['expires_in'], httponly=False)
    return response
...
def is_valid_user(username, password):
    try:
        ...
        logger.warning("failed checking user's username and password: %s" % str(e))
        return False
